...
export default function AddMemberPage() {
    ...
    const [familyId, setFamilyId] = useState(""); // This is the mongo _id of the family
    const { t } = useLanguage();
    ...
    useEffect(() => {
        // Fetch user's family ID to associate with the new member
        const fetchFamilyId = async () => {
            const params = new URLSearchParams(window.location.search);
            const familyIdParam = params.get("familyId");
            if (familyIdParam) {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    if (data && data.length > 0) {
                        ...
                        setError("No family registered. Please register a family first.");
                    }
                }
            } catch (e) {
                setError("Error fetching family to associate.");
            }
        };
        fetchFamilyId();
        fetchFamilyId();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!familyId) {
            setError("Cannot add member without a registered family.");
            return;
        }
        ...
        try {
            ...
            if (!res.ok) {
                if (res.status === 413) {
                    throw new Error("Member photo is too large. Please use an image under 5MB.");
                }
    ...
    return (
        <div className="max-w-3xl mx-auto space-y-6">
            <div className="flex justify-between items-center">
                <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{t("addFamilyMember")}</h1>
            </div>
            ...
            >
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                onChange={handleChange}
                                placeholder={t("motherNameOptional")}
                                className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white"
                            />
                        </div>
                        ...
                        <div>
                            ...
                            {formData.memberPhoto && (
                                <div className="mt-4">
                                    <img src={formData.memberPhoto} alt="Member Preview" className="h-24 w-24 object-cover rounded-xl border border-gray-200 dark:border-gray-600 shadow-sm" />
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="flex justify-end pt-4 border-t dark:border-gray-700">
                        ...
                        >
                            {loading ? t("addingMember") : t("addMemberToFamily")}
                        </button>
                    </div>
                </form>
            </motion.div>
