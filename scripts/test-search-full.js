    ...
    from: (table) => {
        ...
        const builder = {
            ...
            all: async () => {
                ...
                const queryStr = params.toString().replace(/%28/g, '(').replace(/%29/g, ')').replace(/%2C/g, ',');
                console.log(`URL being queried: ${table}?${queryStr}`);
                return await supabaseFetch(`${table}?${queryStr}`, { method: "GET" });
            }
        };
        return builder;
    }
...
async function testFull() {
    try {
        ...
        let familiesRes = await familyQuery.limit(50).all();
        console.log("rawFamilies length", familiesRes.length);

        if (familiesRes.length > 0) {
            const familyIds = familiesRes.map(f => f.id);
            console.log("Fetching members for families: " + familyIds.join(','));
            const membersRes = await supabase.from('Member')
                .select('*')
                .filter('familyId', 'in', `(${familyIds.join(',')})`)
                .all();
            console.log("allMembers length", membersRes.length);
        }
    } catch (e) {
        console.error(e);
    }
